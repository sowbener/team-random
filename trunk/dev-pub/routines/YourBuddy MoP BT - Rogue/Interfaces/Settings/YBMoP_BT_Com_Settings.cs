...
{
    internal class YBSettingsCom : Styx.Helpers.Settings
    {
        public static readonly YBSettingsCom Instance = new YBSettingsCom();

        public YBSettingsCom()
            : base(
                Path.Combine(Utilities.AssemblyDirectory,
                             string.Format(@"Settings/YBMoP/Combat-Settings-{0}-Rev{1}.xml", StyxWoW.Me.Name, YBMain.Revision)))
        {
        }

        [Setting, DefaultValue(AbilityTrigger.Always)]
        public AbilityTrigger AdrenalineRush { get; set; }
        [Setting, DefaultValue(AbilityTrigger.Always)]
        ...
        [Setting, DefaultValue(AbilityTrigger.OnBossDummy)]
        public AbilityTrigger KillingSpree { get; set; }
        [Setting, DefaultValue(AbilityTrigger.OnBossDummy)]
