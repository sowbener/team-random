...
namespace YBMoP_BT_Rogue.Managers
{
    internal static class TalentManager
    {
        ...
        public static void Update()
        {
            ...
            YBLogger.DiagLogW("Glyphdetection - GetNumGlyphSockets {0}", glyphCount);
            ...
            if (glyphCount != 0)
            {
                ...
                for (int i = 1; i <= glyphCount; i++)
                {
                    ...
                    try
                    {
                        if (glyphSpellId > 0)
                        {
                            YBLogger.DiagLogW("Glyphdetection - SpellId: {0},Name:{1} ,WoWSpell: {2}", glyphSpellId, WoWSpell.FromId(glyphSpellId).Name, WoWSpell.FromId(glyphSpellId));
                            Glyphs.Add(WoWSpell.FromId(glyphSpellId).Name.Replace("Glyph of ", ""));
                        }
                        else
                        {
                            YBLogger.DiagLogW("Glyphdetection - Couldn't find all values to detect the Glyph in slot {0}", i);
                        }
                    }
                    catch (Exception ex)
                    {
                        YBLogger.DiagLogW("We couldn't detect your Glyphs");
                        YBLogger.DiagLogW("Report this message to us: " + ex);
                    }
                }
            }
        }
