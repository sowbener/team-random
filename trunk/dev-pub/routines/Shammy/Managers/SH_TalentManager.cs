...
{
    internal static class SmTalentManager
    {
        static SmTalentManager()
        {
        ...
        public static void Update()
        {
            ...
            SmLogger.DiagLogW("Glyphdetection - GetNumGlyphSockets {0}", glyphCount);
            ...
            if (glyphCount != 0)
            {
                ...
                for (int i = 1; i <= glyphCount; i++)
                {
                    ...
                    try
                    {
                        if (glyphSpellId > 0)
                        {
                            SmLogger.DiagLogW("Glyphdetection - SpellId: {0},Name:{1} ,WoWSpell: {2}", glyphSpellId, WoWSpell.FromId(glyphSpellId).Name, WoWSpell.FromId(glyphSpellId));
                            Glyphs.Add(WoWSpell.FromId(glyphSpellId).Name.Replace("Glyph of ", ""));
                        }
                        else
                        {
                            SmLogger.DiagLogW("Glyphdetection - Couldn't find all values to detect the Glyph in slot {0}", i);
                        }
                    }
                    catch (Exception ex)
                    {
                        SmLogger.DiagLogW("We couldn't detect your Glyphs");
                        SmLogger.DiagLogW("Report this message to us: " + ex);
                    }
                }
            }
        }
