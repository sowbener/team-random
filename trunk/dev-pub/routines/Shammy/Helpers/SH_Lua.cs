...
namespace Shammy.Helpers
{
    static class SmLua
    {
        ...
        internal static void PopulateSecondryStats()
        {
            ...
            SmLogger.InfoLog("");
            SmLogger.InfoLog("Health: {0}", StyxWoW.Me.MaxHealth);
            SmLogger.InfoLog("Agility: {0}", StyxWoW.Me.Agility);
            SmLogger.InfoLog("Intellect: {0}", StyxWoW.Me.Intellect);
            SmLogger.InfoLog("Spirit: {0}", StyxWoW.Me.Spirit);
            SmLogger.InfoLog("");
            SmLogger.InfoLog("Attack Power: {0}", _secondaryStats.AttackPower);
            SmLogger.InfoLog("Power: {0:F2}", _secondaryStats.Power);
            SmLogger.InfoLog("Hit(M/R): {0}/{1}", _secondaryStats.MeleeHit, _secondaryStats.SpellHit);
            SmLogger.InfoLog("Expertise: {0}", _secondaryStats.Expertise);
            SmLogger.InfoLog("Mastery: {0:F2}", _secondaryStats.Mastery);
            SmLogger.InfoLog("Mastery (CR): {0:F2}", _secondaryStats.MasteryCR);
            SmLogger.InfoLog("Crit: {0:F2}", _secondaryStats.Crit);
            SmLogger.InfoLog("Haste(M/R): {0} (+{1} % Haste) / {2} (+{3} % Haste)", _secondaryStats.MeleeHaste, Math.Round(_secondaryStats.MeleeHaste / 425.19, 2), _secondaryStats.SpellHaste, Math.Round(_secondaryStats.SpellHaste / 425.19, 2));
            SmLogger.InfoLog("SpellPen: {0}", _secondaryStats.SpellPen);
            SmLogger.InfoLog("PvP Resil: {0}", _secondaryStats.Resilience);
            SmLogger.InfoLog("PvP Power: {0}", _secondaryStats.PvpPower);
            SmLogger.InfoLog("");
        }
        ...
        internal class SecondaryStats
        {
            ...
            public void Refresh()
            {
                ...
                {
                    SmLogger.FailLog(" Lua Failed in SecondaryStats");
                }
        ...
        public static double PlayerPower
        {
            get
            {
                ...
                {
                    SmLogger.DiagLogP("Sm: Lua Failed in PlayerPower"); return 0;
                }
            }
        }
        ...
        public static double PlayerPowerMax
        {
            get
            {
                ...
                {
                    SmLogger.DiagLogP("Sm: Lua Failed in PlayerPowerMax"); return 0;
                }
            }
        }
        #endregion
        ...
        {
            if (SmSettings.Instance.General.CheckDisableClickToMove)
            {
