    ...
    internal static class BMSpell
    {
        ...
        public static Composite CastRaidBuff(string name, CanRunDecoratorDelegate cond)
        {
            return new Decorator(
                delegate(object a)
                {
                    if (!Bubbleman.Interfaces.Settings.BMSettings.Instance.General.EnableRaidPartyBuffing)
                        return false;
                    ...
                    return players.Any(x => x.Distance2DSqr < 40 * 40 && ((!x.HasAnyAura(Bubbleman.Routines.BMGlobal.LegacyoftheWhiteTiger) && Me.Specialization == WoWSpec.PaladinRetribution) || !x.HasAnyAura(Bubbleman.Routines.BMGlobal.LegacyoftheEmperor)) && !x.IsDead && !x.IsGhost && x.IsAlive);
                },
                new Sequence(
        ...
        public static bool CanCastHack(SpellFindResults sfr, WoWUnit unit, bool skipWowCheck = false)
        {
            ...
            double currentPower = BMLua.PlayerPower;
                        ...
                        new Action(ret =>
                            {
                                ...
                                BMLogger.CombatLogO("Casting: " + spell);
                            }
                        )));
        }
        ...
        public static bool IsGlobalCooldown(BMEnum.LagTolerance allow = BMEnum.LagTolerance.Yes)
        {
#if NO_LATENCY_ISSUES_WITH_GLOBAL_COOLDOWN
            uint latency = allow == BMEnum.LagTolerance.Yes ? StyxWoW.WoWClient.Latency : 0;
