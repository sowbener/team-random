        ...
        private void FuInterface_Load(object sender, EventArgs e)
        {
            ...
            switch (StyxWoW.Me.Specialization)
            {
                case WoWSpec.MonkBrewmaster:
                    { selectSpec = internalSettings.Brewmaster; }
                    break;

                case WoWSpec.MonkWindwalker:
                    //if (InternalSettings.Instance.General.CrBrewmasterRotVersion == Enum.RotationVersion.PvP)
                    //{ selectSpec = internalSettings.PvPBrewmaster; }
                    //else
