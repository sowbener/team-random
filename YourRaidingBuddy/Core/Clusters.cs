...
namespace YourRaidingBuddy.Core
{
    ...
    {
        public static IEnumerable<WoWUnit> GetCluster(WoWUnit target, IEnumerable<WoWUnit> otherUnits, Enum.ClusterType type, float clusterRange)
        {
            if (target == null)
            ...
            switch (type)
            {
                case Enum.ClusterType.Radius:
                    return GetRadiusCluster(target, otherUnits, clusterRange);
                case Enum.ClusterType.Chained:
                    return GetChainedCluster(target, otherUnits, clusterRange);
                case Enum.ClusterType.Cone:
                    return GetConeCluster(target, otherUnits, clusterRange);
                case Enum.ClusterType.PathToUnit:
                    return GetPathToUnitCluster(target, otherUnits, clusterRange);
                default:
                    throw new ArgumentOutOfRangeException("type");
            }
        }
        public static int GetClusterCount(WoWUnit target, IEnumerable<WoWUnit> otherUnits, Enum.ClusterType type, float clusterRange)
        {
            if (target == null)
                return 0;
        ...
        public static WoWUnit GetBestUnitForCluster(IEnumerable<WoWUnit> units, Enum.ClusterType type, float clusterRange)
        {
            if (!units.Any())
                return null;
