            ...
            if (SpellManager.FindSpell(spell, out results))
            {
                ...
                try
                {
                    ...
                    if (cooldowns.TryGetValue(result, out lastUsed))
                    {
                        ...
                        {
                            Logger.CooldownTrackerLog("Yb: Checked cooldown for {0} at {1}", spell, DateTime.Now);
                            return result.Cooldown;
                        }
                        Logger.CooldownTrackerLog("Yb: Found {0} but not ready to check at {1}", spell, DateTime.Now);
                        return true;
                    }
                }
                catch
                {
                    Logger.CooldownTrackerLog("Yb: FindSpell Found {0} but currently not in our cooldownlist, first usage?", spell);
                    return false;
                }
            }
            else
            {
                Logger.CooldownTrackerLog("Yb: FindSpell {0} failed at {1}", spell, DateTime.Now);
            }
            return false;
        }
            ...
            if (SpellManager.FindSpell(spell, out results))
            {
                ...
                try
                {
                    ...
                    if (cooldowns.TryGetValue(result, out lastUsed))
                    {
                        ...
                        if (DateTime.Compare(DateTime.Now, lastUsedminusBuffer) > 0)
                        {
                            Logger.CooldownTrackerLog("Yb: Checked CooldownTimeLeft for {0} at {1}", spell, DateTime.Now);
                            return result.CooldownTimeLeft;
                        }

                        Logger.CooldownTrackerLog("Yb: Found {0} but not ready to check CooldownTimeLeft {1}", spell, DateTime.Now);
